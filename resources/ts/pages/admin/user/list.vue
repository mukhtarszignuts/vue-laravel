...
import { fetchCompanyList } from "@/services/GeneralService";
...
// const options = ref({ page: currentPage.value, itemsPerPage: perPage.value, sortBy: [''], sortDesc: [false] })
const selected = ref([]);
...
const isExportButton = ref<boolean>(false);
const { roles, statusOptions, roleVariant, statusVariant } = useFormatting();
...
const AddNewUser = async (userData: any) => {
  try {
    ...
    //   const { data } = await axios.post("admin/user/update", formData, {
    //     headers: {
    ...
    //   response = data;
    // } else {
    //   const { data } = await axios.post("admin/user/create", formData, {
    //     headers: {
    //       "Content-Type": "multipart/form-data",
    //     },
    //   });
...
const exportCsv = async () => {
  try {
    isExportButton.value = false;
    // fullScreenLoad.value = true;
    const input = {
      start_date: filerDate.value
        ? filerDate.value.includes("to")
          ? filerDate.value.split(" to ")[0]
...
<template>
  <div>
            ...
            <VCol md="9" lg="10" sm="12" cols="12">
              <div class="d-flex gap-3 common-filter justify-end">
                ...
                <div class="d-flex common-filter gap-3">
                  <VBtn
                    :disabled="!roleFilter && !statusFilter && !filerDate"
                    @click="ResetFilter"
                  >
                    Reset Filter
                  </VBtn>
                  <div class="gap-3 d-flex flex-column">
                    <div class="d-flex gap-3 flex-wrap">
                      ...
                      <VBtn
                        v-if="userData?.role === 'A'"
                        @click="isExportButton = true"
                      >
                        Export
                      </VBtn>
                      <!-- <VBtn v-if="userData?.role === 'A'" @click="true">
                        Import
                      </VBtn> -->
                    </div>
                  </div>
              ...
              <tr v-for="(item, index) in userList" :key="item.id">
                ...
                <td>
                  <div class="d-flex align-center">
                    <VAvatar
                      size="38"
                      :variant="!item?.company ? 'tonal' : undefined"
                      :color="'secondary'"
                      class="me-3"
                    >
                      <VImg
                        v-if="item?.company?.image_url"
                        :src="item?.company?.image_url"
                      />
                      <span v-else>{{ avatarText(item?.company?.name) }}</span>
                    </VAvatar>
                    <div class="d-flex flex-column">
                      ...
                      <span class="text-sm text-disabled">
                        ...
                        <VChip
                          label
                          :color="roleVariant(item.role).color"
    ...
    <AddEditUserDialog
      ...
      :user-data="addEditUserData"
      :roles="roles"
      :status="statusOptions"
      :compaines="companyList"
      @close-dialog="isAddNewUserDrawerVisible = false"
      @user-data="AddNewUser"
    />

    <!-- Export CSV -->
    <VDialog v-model="isExportButton" persistent class="v-dialog-sm">
      <!-- Dialog close btn -->
      <DialogCloseBtn @click="isExportButton = !isExportButton" />
      <!-- Dialog Content -->
      <VCard>
        <VCardText>
          <div class="text-center mt-5">
            <h2>Are You Sure You Want to Export Csv Data ?</h2>
          </div>
        </VCardText>
        <VCardText class="d-flex justify-center gap-3 flex-wrap">
          <VBtn color="error" variant="tonal" @click="isExportButton = false">
            No
          </VBtn>
          <VBtn @click="exportCsv()" color="success"> Download </VBtn>
        </VCardText>
      </VCard>
    </VDialog>
  </div>
...
.table-container table {
  width: 100%;
  min-width: 800px; /* Adjust this to fit your table's minimum width */
}

.table-container th,
.table-container td {
  white-space: nowrap; /* Prevents content from wrapping and maintains a single line */
}
</style>
